...
function Filter() {
    ...
    // Function to handle filter price change
    ...
    // Filtered products based on search key, filter type, and filter price
    const filteredProducts = product.filter(item => {
        const itemName = item.name || ''; // Ensure itemName is a string
        const matchSearchKey = itemName.toLowerCase().includes(searchkey.toLowerCase());
        const matchFilterType = !filterType || item.category === filterType;
        const matchFilterPrice = !filterPrice || parseFloat(item.price) <= parseFloat(filterPrice);
        return matchSearchKey && matchFilterType && matchFilterPrice;
    });
    ...
    return (
      <div>
        <div className="container mx-auto px-1 mt-5">
          <div
            className="p-5 rounded-lg bg-gray-300 drop-shadow-xl border border-gray-400"
            style={{
